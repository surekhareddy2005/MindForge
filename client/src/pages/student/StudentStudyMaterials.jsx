...
const StudentStudyMaterials = () => {
  ...
  const calendarGuides = guides.filter(guide => {
  ...
  const filteredGuides = calendarGuides.filter(guide => {
    return !selectedDate || (
  ...
  const hasGuideOnDay = (day) => {
    return calendarGuides.some(guide =>
      guide.date.getDate() === day &&
      guide.date.getMonth() === currentDate.getMonth() &&
      guide.date.getFullYear() === currentDate.getFullYear()
    );
  };
    ...
    return (
      <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-main)' }}>
        ...
        <main style={{ flex: 1, padding: '2rem 3rem' }}>
          <Header title="Study Materials" subtitle="Loading your materials..." />
          <DotsLoader />
        </main>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-main)' }}>
      <StudentSidebar />
      <main style={{ flex: 1, padding: '2rem 3rem' }}>
        <Header title="Study Materials" subtitle="Access all your AI-generated learning content" />
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
            ...
            <CustomSelect 
              ...
              options={['All Modules', ...(selectedCourse !== 'All Courses' ? availableModules[selectedCourse] || [] : [])]}
          ...
          {selectedDate && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.5rem', padding: '8px 16px', background: 'var(--primary-surface)', borderRadius: '8px', width: 'fit-content' }}>
              ...
              <button onClick={() => setSelectedDate(null)} style={{ background: 'none', border: 'none', color: 'var(--text-primary)', cursor: 'pointer', marginLeft: '8px', fontSize: '1rem' }}>x</button>
            </div>
          )}
          ...
          {currentGuides.length === 0 ? (
            <EmptyState
              ...
              subtitle={selectedDate ? 'There are no study guides generated for this specific date.' : 'Try adjusting your search terms or open a course session with uploaded study materials.'}
          ...
          <div className="glass" style={{ padding: '1.5rem', borderRadius: '24px', position: 'sticky', top: '100px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px', textAlign: 'center' }}>
              {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(day => (
                <div key={day} style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 600, paddingBottom: '8px' }}>{day}</div>
              ))}
